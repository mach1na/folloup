...
namespace font_renderer_detail {
...
inline int ScaleMetric(int value, const FontSelection& selection)
{
...
inline FontSelection MakeFontSelection(const design::TypographySpec& spec,
                                       int base_size,
...
inline FontSelection FontForRole(design::TypographyRole role)
{
...
// DrawPixelFn is a template parameter (rather than a std::function) so the compiler can inline
// the actual pixel write -- this runs on every glyph of every string on every screen redraw.
